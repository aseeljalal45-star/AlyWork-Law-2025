...
def show_workers_section():
    st.markdown("### 👷 قسم العمال - حقوقك القانونية")
    
    tab1, tab2, tab3 = st.tabs(["📋 الحقوق الأساسية", "🧮 الحاسبات", "📞 الدعم"])
    
    with tab1:
        st.markdown("#### 📋 حقوقك الأساسية")
        
        rights = [
            {"right": "عقد عمل مكتوب", "law": "المادة 13", "details": "يجب توقيع عقد قبل بدء العمل"},
            {"right": "الحد الأدنى للأجور", "law": "المادة 35", "details": "290 دينار للمؤهلين - 2025"},
            {"right": "موعد صرف الراتب", "law": "المادة 55", "details": "خلال 7 أيام من نهاية الشهر"},
            {"right": "بدل العمل الإضافي", "law": "المادة 42", "details": "125% نهاري، 150% ليلي وعطلات"},
            {"right": "الإجازة السنوية", "law": "المادة 52", "details": "14-28 يوم حسب مدة الخدمة"},
            {"right": "إجازة الأمومة", "law": "المادة 69", "details": "10 أسابيع مدفوعة الأجر - 2025"}
        ]
        
        for item in rights:
            with st.expander(f"✅ {item['right']}"):
                st.write(f"**القانون:** {item['law']}")
                st.write(f"**التفاصيل:** {item['details']}")
    
    with tab2:
        st.markdown("#### 🧮 حاسبات المستحقات")
        
        col1, col2 = st.columns(2)
        
        with col1:
            salary = st.number_input("الراتب الأساسي (دينار)", min_value=290, value=500)
            years = st.number_input("عدد السنوات", min_value=0, max_value=40, value=3)
        
        with col2:
            termination_type = st.selectbox("سبب إنهاء الخدمة", ["استقالة", "إنهاء من صاحب العمل"])
            overtime_hours = st.number_input("ساعات العمل الإضافي", value=10)
        
        if st.button("🧮 احسب المستحقات"):
            # حساب مبسط
            end_service = (salary * years * 12) / 24 if termination_type == "استقالة" else (salary * years * 12) / 12
            overtime_pay = overtime_hours * (salary / 30 / 8) * 1.25
            
            st.success(f"""
            **النتائج:**
            - مكافأة نهاية الخدمة: {end_service:,.0f} دينار
            - بدل العمل الإضافي: {overtime_pay:,.0f} دينار
            - الإجمالي: {end_service + overtime_pay:,.0f} دينار
            """)
    ...
    with tab3:
        ...
        **جهات الاتصال:**
        ...
        - 🏛️ مديريات العمل في جميع المحافظات
        - ⚖️ محاكم العمل المختصة
        - 🤝 نقابات العمال
        """)
...
# ==========================
# 👔 قسم أصحاب العمل الأساسي
# ==========================
def show_employers_section():
    st.markdown("### 👔 قسم أصحاب العمل - الالتزامات القانونية")
    
    tab1, tab2, tab3 = st.tabs(["📋 الالتزامات", "🔍 الامتثال", "📊 الإدارة"])
    
    with tab1:
        st.markdown("#### 📋 الالتزامات الأساسية")
        
        obligations = [
            {"obligation": "عقود عمل مكتوبة", "law": "المادة 13", "deadline": "قبل بدء العمل"},
            {"obligation": "الحد الأدنى للأجور", "law": "المادة 35", "deadline": "مستمر - 290 دينار 2025"},
            {"obligation": "تسجيل الضمان الاجتماعي", "law": "المادة 81", "deadline": "خلال 7 أيام من التعيين"},
            {"obligation": "بيئة عمل آمنة", "law": "المادة 79", "deadline": "مستمر"},
            {"obligation": "صرف الرواتب", "law": "المادة 55", "deadline": "أول 7 أيام من الشهر"}
        ]
        
        for item in obligations:
            with st.expander(f"📋 {item['obligation']}"):
                st.write(f"**القانون:** {item['law']}")
                st.write(f"**الموعد النهائي:** {item['deadline']}")
    
    with tab2:
        st.markdown("#### 🔍 مدقق الامتثال السريع")
        
        checks = [
            "هل توجد عقود عمل مكتوبة لجميع العاملين؟",
            "هل يتم تطبيق الحد الأدنى للأجور 290 دينار؟",
            "هل يتم صرف الرواتب في الموعد المحدد؟",
            "هل يتم تسجيل العاملين في الضمان الاجتماعي؟"
        ]
        
        score = 0
        for check in checks:
            if st.radio(check, ["نعم", "لا"], horizontal=True) == "نعم":
                score += 1
        
        st.progress(score / len(checks))
        st.write(f"**معدل الامتثال: {score}/{len(checks)}**")
...
def show_graduates_guide():
    ...
    tabs = st.tabs(["بداية المسيرة", "نصائح عملية", "جهات الدعم"])
...
# ==========================
# 🔬 قسم الباحثين الأساسي
# ==========================
def show_researchers_section():
    st.markdown("### 🔬 قسم الباحثين")
    
    tab1, tab2 = st.tabs(["📚 التشريعات", "⚖️ السوابق"])
    
    with tab1:
        st.markdown("#### 📚 التشريعات الأساسية")
        
        laws = [
            {"name": "قانون العمل الأردني", "year": "1996", "updates": "2025"},
            {"name": "نظام الأجور والبدلات", "year": "2025", "updates": "أحدث"},
            {"name": "تعليمات العمل عن بعد", "year": "2025", "updates": "جديد"},
            {"name": "نظام الضمان الاجتماعي", "year": "2014", "updates": "2024"}
        ]
        
        for law in laws:
            st.write(f"**{law['name']}** ({law['year']}) - {law['updates']}")
    
    with tab2:
        st.markdown("#### ⚖️ سوابق قضائية هامة")
        
        cases = [
            {"case": "قضية الفصل التعسفي", "year": "2024", "principle": "تعويض عن الفصل بدون مبرر"},
            {"case": "قضية التمييز في الأجور", "year": "2023", "principle": "المساواة في الأجر عن العمل المتساوي"}
        ]
        
        for case in cases:
            with st.expander(f"{case['case']} - {case['year']}"):
                st.write(f"**المبدأ:** {case['principle']}")

# ==========================
# ⏰ منبه المواعيد الأساسي
# ==========================
def show_reminder_system():
    st.markdown("### ⏰ منبه المواعيد")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("#### إضافة تذكير")
        
        with st.form("add_reminder"):
            title = st.text_input("عنوان التذكير")
            due_date = st.date_input("التاريخ")
            priority = st.selectbox("الأولوية", ["منخفضة", "متوسطة", "عالية"])
    ...
    with col2:
        ...
        if 'reminders' in st.session_state and st.session_state.reminders:
            for reminder in st.session_state.reminders:
                st.write(f"**{reminder['title']}** - {reminder['due_date']}")
        else:
            st.info("لا توجد تذكيرات")

# ==========================
# 🗺️ خريطة الحقوق الأساسية
# ==========================
def show_rights_map():
    st.markdown("### 🗺️ خريطة الحقوق")
    
    role = st.radio("اختر دورك:", ["عامل", "صاحب عمل"], horizontal=True)
    
    if role == "عامل":
        st.markdown("#### 👷 حقوق العامل")
        rights = {
            "💰 مالية": ["الحد الأدنى للأجور", "بدل العمل الإضافي", "مكافأة نهاية الخدمة"],
            "⏰ وقت العمل": ["ساعات العمل", "الإجازات", "فترات الراحة"],
            "🛡️ حماية": ["السلامة المهنية", "الحماية من التحرش", "التأمين الصحي"]
        }
        
        for category, items in rights.items():
        ...
        obligations = {
            "📝 تعاقد": ["عقود عمل", "تسجيل ضمان", "لوائح عمل"],
            "💰 مالية": ["صرف رواتب", "تسديد اشتراكات", "توفير بدلات"],
            "🛡️ سلامة": ["بيئة آمنة", "معدات وقاية", "تدريب سلامة"]
        }
...
def show_legal_search():
    st.markdown("### 🔍 البحث في القوانين")
    
    search_term = st.text_input("اكتب مصطلح للبحث:")
    
    if search_term:
        # محاكاة نتائج البحث
        results = {
            "أجر": ["المادة 35: الحد الأدنى للأجور", "المادة 42: بدل العمل الإضافي"],
            "إجازة": ["المادة 52: الإجازة السنوية", "المادة 69: إجازة الأمومة"],
            "فصل": ["المادة 72: إنهاء الخدمة", "المادة 73: الفصل التعسفي"]
        }
        
        for term, laws in results.items():
...
def show_authorities_section():
    st.markdown("### 🏛️ الجهات المختصة")
    
    authorities = {
        "عمان": {
            "وزارة العمل": "06-5802666",
            "محكمة العمل": "06-5651900",
            "الضمان الاجتماعي": "06-5521221"
        },
        "إربد": {
            "مديرية العمل": "02-7272111"
        }
    }
    
    selected_gov = st.selectbox("اختر المحافظة", list(authorities.keys()))
    
    if selected_gov:
        for authority, phone in authorities[selected_gov].items():
